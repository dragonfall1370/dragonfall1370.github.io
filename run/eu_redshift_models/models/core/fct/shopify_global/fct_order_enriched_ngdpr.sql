  ...
  create view "airup_eu_dwh"."shopify_global"."fct_order_enriched_ngdpr__dbt_tmp" as (
    ---Authors: Etoma Egot
---Last Modified by: Yu-Shih Hsieh (add SE)

---###################################################################################################################

        ---This query adds country mapping information to shopify_global."orders" as well as precalculations for
        ---This is the main enriched order model which combines data from all D2C webshops in a non-GDPR compliant manner---
   ...
   ---Note: This is to be used only with approval from Head of Data, requesting dept head-of and CTO.
---###################################################################################################################
...
main as (
    select
        ...
        case
            when "order".financial_status in ('paid', 'partially_refunded')
        ...
        (net_revenue_1
            * case
                when (currency_rates_usd.conversion_rate_eur is null) and (date_trunc('year', created_at_premain) = '2019-01-01') then 1.120129
                when (currency_rates_usd.conversion_rate_eur is null) and (date_trunc('year', created_at_premain) = '2020-01-01') then 1.142203
                when (currency_rates_usd.conversion_rate_eur is null) and (date_trunc('year', created_at_premain) = '2021-01-01') then 1.18318
        ...
        case
                when lower("order".billing_address_country::text) = 'netherlands'::text THEN 'NL'::text
                when lower("order".billing_address_country::text) = 'belgium'::text THEN 'BE'::text
                when lower("order".billing_address_country::text) = 'france'::text THEN 'FR'::text
                when lower("order".billing_address_country::text) = 'switzerland'::text THEN 'CH'::text
                when lower("order".billing_address_country::text) = 'united kingdom'::text THEN 'UK'::text
                when lower("order".billing_address_country::text) = 'italy'::text THEN 'IT'::text
                when lower("order".billing_address_country::text) = 'austria'::text THEN 'AT'::text
                when lower("order".billing_address_country::text) = 'germany'::text THEN 'DE'::text
                when lower("order".billing_address_country::text) = 'sweden'::text THEN 'SE'::text
                when lower("order".billing_address_country::text) = 'united states'::text THEN 'US'::text
                else 'others'::text
            end as shop_country
    from
        premain "order"
    left join "airup_eu_dwh"."public"."country_system_account_mapping" csam ON "order".shipping_address_country::varchar = (csam.shopify_shipping_address_country)
    ...
    left join odoo_currency.dim_global_currency_rates currency_rates_usd
        on date("order".created_at_premain) = currency_rates_usd.creation_date
