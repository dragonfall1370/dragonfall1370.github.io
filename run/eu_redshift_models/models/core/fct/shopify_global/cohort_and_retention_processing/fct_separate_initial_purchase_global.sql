  ...
  create view "airup_eu_dwh"."shopify_global"."fct_separate_initial_purchase_global__dbt_tmp" as (
    --legacy:  shopify_global.shopify_cohort_processing_separate_initial_purchase_global
...
WITH product_categories_per_order AS (
         ...
         SELECT DISTINCT foe.customer_id,
            ...
            foe.created_at,
                CASE
                    ...
                    AND product_categories_per_order.starter_sets >= 1 THEN 1
                    ELSE 0
                END AS init_order_starter_set,
                CASE
                    WHEN sum(1) OVER (PARTITION BY foe.customer_id ORDER BY foe.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) = 1 
                    AND product_categories_per_order.pods >= 1 THEN 1
                    ELSE 0
            ...
            min(date(date_trunc('month', foe.created_at))) OVER (PARTITION BY foe.customer_id) AS cohort,
            date_trunc('month', foe.created_at) AS month_created_at,
            -- ##############################
           ...
           foe.order_number,
            foe.created_at,
             product_categories_per_order.starter_sets,
              product_categories_per_order.pods,
               product_categories_per_order.loops
          ORDER BY foe.customer_id, foe.created_at
        ), 
        data_prep_2 AS (
         SELECT customer_id,
                order_number,
                ...
                month_created_at,
                CASE
                    WHEN nth_order = 1 THEN '-1'::integer
        ...
        data_prep_3 AS (
         SELECT customer_id,
           ...
           FROM data_prep_2
          GROUP BY customer_id, cohort, date_diff_months
        ), 
        returning_customers AS (
         SELECT cohort,
           ...
           FROM data_prep_2
          GROUP BY cohort, init_order_incl_starter_set, init_order_incl_pods
        )
