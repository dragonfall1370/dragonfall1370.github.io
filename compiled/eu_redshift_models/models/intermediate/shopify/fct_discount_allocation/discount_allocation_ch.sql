  ...
  SELECT 
    ...
    discount_allocation.amount AS amount_chf,
    ...
    discount_allocation.amount_set_presentment_money_amount AS amount_set_presentment_money_amount_chf,
    ...
    discount_allocation.amount_set_shop_money_amount AS amount_set_shop_money_amount_chf,
   ...
   FROM "airup_eu_dwh"."shopify_ch"."discount_allocation"
  ...
  discount_allocation_enriched as (
   ...
   select
    ...
    (discount_allocation_raw.amount / case when global_curr_chf.conversion_rate_eur is not null then global_curr_chf.conversion_rate_eur else 1.0986 end)::numeric(10,3)::double precision as amount,
    discount_allocation_raw.amount_chf,
    discount_allocation_raw.amount_gbp,
	discount_allocation_raw.amount_sek,
   (discount_allocation_raw.amount_set_presentment_money_amount / case when global_curr_chf.conversion_rate_eur is not null then global_curr_chf.conversion_rate_eur else 1.0986 end)::numeric(10,3)::double precision as amount_set_presentment_money_amount,
   ...
   (discount_allocation_raw.amount_set_shop_money_amount / case when global_curr_chf.conversion_rate_eur is not null then global_curr_chf.conversion_rate_eur else 1.0986 end)::numeric(10,3)::double precision as amount_set_shop_money_amount,
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."discount_allocation_ch")
