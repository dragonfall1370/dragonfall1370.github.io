  ...
  select
    ...
    product_variant.compare_at_price AS compare_at_price_gbp,
    ...
    product_variant.price AS price_gbp,
  ...
  FROM "airup_eu_dwh"."shopify_uk"."product_variant"
  ...
  select
    ...
    (product_variant_raw.compare_at_price / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as compare_at_price,										  
   ...
   (product_variant_raw.price / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as price,							   
  ...
  select * from product_variant_enriched
   ...
   where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."product_variant_uk")
