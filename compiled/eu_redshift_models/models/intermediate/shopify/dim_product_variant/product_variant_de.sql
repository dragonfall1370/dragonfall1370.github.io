--incrementing the model and specifiyng the target destination of the table in the DWH---
   ...
   ----extracting relevant fields 
	   ...
	   'Base'::text AS shopify_shop,            ---add a placeholder to identify legacy webshop
	   'EUR'::text as currency_abbreviation     ---add a placeholder for euro currency 

   FROM "airup_eu_dwh"."shopify_de"."product_variant"
  ...
  select * from product_variant_enriched
   ...
   where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."product_variant_de")
