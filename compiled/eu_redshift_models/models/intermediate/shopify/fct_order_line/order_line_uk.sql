   ...
   with 
    ...
    select
		    ...
		    pre_tax_price,
		    order_line.pre_tax_price AS pre_tax_price_gbp,
		    NULL::double precision AS pre_tax_price_chf,
			NULL::double precision AS pre_tax_price_sek,
		    order_line.pre_tax_price_set,
		    price,
		    NULL::double precision AS price_chf,
		    order_line.price AS price_gbp,
		    ...
		    order_line.title,
		    total_discount,
		    NULL::double precision AS total_discount_chf,
		    order_line.total_discount AS total_discount_gbp,
	 ...
	 FROM "airup_eu_dwh"."shopify_uk"."order_line"
  ...
  order_line_enriched as (
   ...
   select
		    ...
		    (order_line_raw.pre_tax_price / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as pre_tax_price,
		    ...
		    (order_line_raw.price / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as price,
		    ...
		    (order_line_raw.total_discount / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as total_discount,
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_line_uk")
