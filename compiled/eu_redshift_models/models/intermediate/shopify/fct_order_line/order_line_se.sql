   ...
   with 
    ...
    select
		    ...
		    order_line.title,
		    total_discount,
            order_line.total_discount AS total_discount_sek,
	 ...
	 FROM "airup_eu_dwh"."shopify_se"."order_line"
  ...
  order_line_enriched as (
   ...
   select
		    ...
		    (order_line_raw.pre_tax_price / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as pre_tax_price,						 
		    ...
		    (order_line_raw.price / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as price,				 
		    ...
		    (order_line_raw.total_discount / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as total_discount,
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_line_se")
