   ...
   FROM "airup_eu_dwh"."shopify_ch"."tax_line"
   ),
  ...
  tax_line_enriched as (
   ...
   select
      tax_line_raw._fivetran_synced,
	    ...
	    (tax_line_raw.price / case when global_curr_chf.conversion_rate_eur is not null then global_curr_chf.conversion_rate_eur else 1.0986 end)::numeric(10,3)::double precision as price ,    							   
	    tax_line_raw.price_chf,
	    tax_line_raw.price_gbp AS price_gbp,
		  tax_line_raw.price_sek AS price_sek,
	    tax_line_raw.price_set,
	    (tax_line_raw.rate / case when global_curr_chf.conversion_rate_eur is not null then global_curr_chf.conversion_rate_eur else 1.0986 end)::numeric(10,3)::double precision as rate, 
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."tax_line_ch")
