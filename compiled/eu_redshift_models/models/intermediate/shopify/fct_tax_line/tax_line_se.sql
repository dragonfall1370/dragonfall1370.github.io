   ...
   FROM "airup_eu_dwh"."shopify_se"."tax_line"
  ...
  tax_line_enriched as (
   ...
   select
	    ...
	    (tax_line_raw.price / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as price,
	   ...
	   (tax_line_raw.rate / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as rate,
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."tax_line_se")
