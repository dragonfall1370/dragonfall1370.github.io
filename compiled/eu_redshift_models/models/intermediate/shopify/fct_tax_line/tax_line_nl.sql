   ...
   WITH tax_line_raw AS (
         ...
         SELECT tax_line._fivetran_synced,
           ...
           FROM "airup_eu_dwh"."shopify_nl"."tax_line"
 ...
 SELECT *
   FROM tax_line_enriched

     -----incrememntal table macro---
  
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."tax_line_nl")
