 ...
 order_shipping_line_raw AS (
		 ...
		 SELECT 
           ...
           FROM "airup_eu_dwh"."shopify_it"."order_shipping_line"
 ...
 SELECT *
   FROM order_shipping_line_enriched

   -----incrememntal table macro---
  
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_shipping_line_it")
