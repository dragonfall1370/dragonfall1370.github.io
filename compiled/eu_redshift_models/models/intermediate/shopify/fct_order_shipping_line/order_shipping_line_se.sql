  ...
  SELECT 
    ...
    order_shipping_line.discounted_price AS discounted_price_sek,
    ...
    order_shipping_line.price AS price_sek,
   ...
   FROM "airup_eu_dwh"."shopify_se"."order_shipping_line"
  ...
  order_shipping_line_enriched as (
   ...
   select
    ...
    (order_shipping_line_raw.discounted_price / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as discounted_price,											  
    ...
    (order_shipping_line_raw.price / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as price,									   
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_shipping_line_se")
