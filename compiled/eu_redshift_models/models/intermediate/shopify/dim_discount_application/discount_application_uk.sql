 ...
 with final as(
   ...
   select
		...
		discount_application.value AS value_gbp,
   ...
   FROM "airup_eu_dwh"."shopify_uk"."discount_application"
  ...
  discount_application_enriched as (
   ...
   select
        ...
        (case when global_curr_gbp.conversion_rate_eur is not null 
         then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as value,
...
-----incremental table macro---
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."discount_application_uk")
