...
with final as(
   ...
   select
		...
		discount_application.value AS value_sek,
   ...
   FROM "airup_eu_dwh"."shopify_se"."discount_application"
  ...
  discount_application_enriched as (
   ...
   select
        ...
        discount_application_raw.order_id,
         ...
         (case when global_curr_sek.conversion_rate_eur is not null 
         then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as value,
...
-----incremental table macro---
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."discount_application_se")
