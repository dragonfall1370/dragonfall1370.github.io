 ...
 select
    ...
    order_line_refund.subtotal AS subtotal_gbp,
    ...
    order_line_refund.total_tax AS total_tax_gbp,
   ...
   FROM "airup_eu_dwh"."shopify_uk"."order_line_refund"
  ...
  order_line_refund_enriched as (
   ...
   select   
	...
	(order_line_refund_raw.subtotal / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as subtotal,
	...
	(order_line_refund_raw.total_tax / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as total_tax,
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_line_refund_uk")
