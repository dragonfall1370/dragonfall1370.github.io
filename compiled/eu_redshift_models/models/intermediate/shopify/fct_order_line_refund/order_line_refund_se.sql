 ...
 select
    ...
    order_line_refund.subtotal AS subtotal_sek,
    ...
    order_line_refund.total_tax AS total_tax_sek,
   ...
   FROM "airup_eu_dwh"."shopify_se"."order_line_refund"
  ...
  order_line_refund_enriched as (
   ...
   select   
	...
	(order_line_refund_raw.subtotal / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as subtotal,
	...
	(order_line_refund_raw.total_tax / case when global_curr_sek.conversion_rate_eur is not null then global_curr_sek.conversion_rate_eur else 10.69 end)::numeric(10,3)::double precision as total_tax,
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_line_refund_se")
