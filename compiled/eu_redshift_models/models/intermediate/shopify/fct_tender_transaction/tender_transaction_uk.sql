...
WITH tender_transaction_raw AS (
         SELECT 
           ...
           FROM "airup_eu_dwh"."shopify_uk"."tender_transaction"
        ...
        global_curr_gbp AS (
         SELECT date(creation_datetime) AS creation_date,
          ...
          WHERE currency_abbreviation::text = 'GBP'::text
        ), 
 ...
 SELECT        _fivetran_synced,
    ...
    -----incrememntal table macro---
            ...
            where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."tender_transaction_uk")
