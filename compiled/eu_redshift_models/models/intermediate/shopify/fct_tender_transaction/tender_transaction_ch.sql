 ...
 WITH tender_transaction_raw AS (
         ...
         SELECT _fivetran_synced,
           ...
           FROM "airup_eu_dwh"."shopify_ch"."tender_transaction"
        ),
         ...
         SELECT date(creation_datetime) AS creation_date,
          ...
          WHERE currency_abbreviation::text = 'CHF'::text
        ), 
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."tender_transaction_ch")
