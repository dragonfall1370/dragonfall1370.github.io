   ...
   select
    ...
    order_shipping_tax_line.price AS price_gbp,
 ...
 select
   ...
   (order_shipping_tax_line_raw.price / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as price, 
   ...
   (order_shipping_tax_line_raw.rate / case when global_curr_gbp.conversion_rate_eur is not null then global_curr_gbp.conversion_rate_eur else 0.863 end)::numeric(10,3)::double precision as rate, 
...
left join global_curr_gbp using (currency_abbreviation, creation_date)
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_shipping_tax_line_uk")
