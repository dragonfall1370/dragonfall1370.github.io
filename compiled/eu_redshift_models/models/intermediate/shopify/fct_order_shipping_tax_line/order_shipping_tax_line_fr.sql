   ...
   FROM "airup_eu_dwh"."shopify_fr"."order_shipping_tax_line"
...
left join global_curr_eur using (currency_abbreviation, creation_date)
  ...
  where _fivetran_synced >= (select max(_fivetran_synced) from "airup_eu_dwh"."shopify_global"."order_shipping_tax_line_fr")
