...
WITH total AS (
	...
	SELECT DISTINCT 
		...
		END AS country
		, CASE WHEN dim_person.custom_first_purchase_date IS NULL THEN 'prospect' ELSE 'customer' END AS customer_status
	FROM klaviyo_global.dim_person
	LEFT JOIN "airup_eu_dwh"."dbt_ohigsonspence"."klaviyo_suppressed" ks ON dim_person.email = ks.email AND custom_sms_consent_date < ks."timestamp" 
	WHERE suppressed IS NULL 
), metrics AS (
	SELECT 
	--	date_trunc('day',updated)::date AS updated_at,
		CASE WHEN custom_sms_consent_date IS NOT NULL THEN date_trunc('week',custom_sms_consent_date)::date ELSE '2022-06-27'::date END AS week
		, total.total
		, suppressed.country
		, suppressed.customer_status
		, SUM(CASE WHEN custom_sms_consent = TRUE THEN 1 ELSE 0 END) AS opt_in
		, SUM(CASE WHEN custom_sms_consent = TRUE THEN 1 ELSE 0 END) / total.total::double precision AS penetration
	FROM suppressed
	LEFT JOIN total ON total.country = suppressed.country AND total.customer_status AND suppressed.customer_status
	GROUP BY CASE WHEN custom_sms_consent_date IS NOT NULL THEN date_trunc('week',custom_sms_consent_date)::date ELSE '2022-06-27'::date END, total, suppressed.country, suppressed.customer_status
)
SELECT 
	week
	, opt_in
