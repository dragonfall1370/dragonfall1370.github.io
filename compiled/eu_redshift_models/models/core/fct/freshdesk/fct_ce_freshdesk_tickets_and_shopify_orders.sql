...
--/*#################################################################
--this view combines freshdesk tickets and shopify orders in order to join the tables correctly and
--allow for the needed analysis in Tableau, several spines (date, country) had to be created.
--#################################################################*/
...
-- test
     incidents_spine as (
         SELECT DISTINCT
            ...
            END as country
         FROM
             "airup_eu_dwh"."freshdesk"."stg_freshdesk_ticket" f
