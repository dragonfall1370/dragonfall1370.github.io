...
select
	...
	spsm.carrier,
	case
		when spsm.country = 'United Kingdom of Great Britain and Northern Ireland' then 'United Kingdom'
		else spsm.country
	end as country,
...
select
	...
	row_number() over (partition by sale_order.name
order by
	(
	case
...
sale_order as (
select
	sale_order_raw.order_name,
	REGEXP_SUBSTR(sale_order_raw.warehouse_id,'[(](.*)[)]',	1,1,'e') as warehouse_id
from sale_order_raw
where
	sale_order_raw.row_num = 1
        ),    
        combined_data as 
        (
select
...
select
	...
	case
		when (date_part(dow, order_datetime) in (0, 6))
			or (date_part(dow, first_delivery_time) in (0, 6))
    then count(*)-1
			else count(*) -1
...
select
	...
	case
		when (date_part(dow, order_datetime) in (0, 6))
			or (date_part(dow, arrived_at_pickup_point) in (0, 6))
    then count(*)-1
			else count(*) -1
