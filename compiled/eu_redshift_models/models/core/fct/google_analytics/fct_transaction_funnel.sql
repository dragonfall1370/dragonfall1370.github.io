...
---###################################################################################################################
        -- this view contains the information on the Google Analytics session with transaction stages
---###################################################################################################################
...
SELECT fp.date,
  ...
  WHERE (fp.shopping_stage::text = 'ALL_VISITS'::text OR fp.shopping_stage::text = 'PRODUCT_VIEW'::text OR fp.shopping_stage::text = 'ADD_TO_CART'::text OR fp.shopping_stage::text = 'TRANSACTION'::text OR fp.shopping_stage::text = 'CHECKOUT'::text OR fp.shopping_stage::text = 'CHECKOUT_1'::text OR fp.shopping_stage::text = 'CHECKOUT_2'::text OR fp.shopping_stage::text = 'CHECKOUT_3'::text OR fp.shopping_stage::text = 'CHECKOUT_4'::text) AND fp.profile::text <> '224422635'::text
