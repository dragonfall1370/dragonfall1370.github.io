...
with
	...
	main_groupby as
		(select
			...
			count(DISTINCT data_prep_2_by_country."oid")  AS orders
		from
			data_prep_2_by_country
        WHERE data_prep_2_by_country.cohort >= '2020-04-01'::date
           AND (data_prep_2_by_country.nth_order >= 2 OR
                data_prep_2_by_country.nth_order = 1 AND data_prep_2_by_country.pods >= 1::double precision)
		group by
            country_abbreviation,
			country_fullname,
			cohort,
	...
	one_side_groupby1 as
		...
		(select
			...
			cohort,
		    date,			
			'all'::text as init_order_incl_starter_set, -- dim 2
			init_order_incl_pods::text,
			sum(orders) as orders
		from
			main_groupby
		group by
	...
	full_agg as
		...
		union_cte as (
			...
			union all
			select * from one_side_groupby1
			union all
			select * from one_side_groupby2
			union all
