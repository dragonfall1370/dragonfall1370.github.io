	...
	WITH
		...
		all_orders_content as (
			select
				...
				case when category = 'Hardware' and subcategory_2 not like '%VSS%' then 'Flavour' else category end as category,
...
-- 			where
-- 				customer_id = '001a154f791852b017136be0c348d8e5'
		...
		first_order_per_product as (
			...
			group by 1,2,3,4,5
			having category = 'Flavour'
		),

		prep as (select customer_id, created_at, ss_bundle_init_order, product, quantity from all_orders_content group by 1,2,3,4,5),
	...
	---#####################################
		first_order_per_product_and_subsequent_orders as (
			select
                ...
                first_order_per_product_enhanced.quantity first_order_quantity,
                ...
                case
					...
					then all_orders_content.quantity else 0
				end as subsequent_quantity,
				case
					when product is not null
					and first_product = product
					then 1 else 0
			...
			where
				all_orders_content.created_at > first_order_per_product_enhanced.first_created_at
		),
	...
	---#####################################
		...
		cohort_size as (
			select
				first_created_at::date as first_created_at,
				first_country,
				first_region,
				first_product,
		 ...
		 final as (
			select
				...
				b.cohort_size,
                b.first_order_quantity,
				sum(subsequent_quantity) as subsequent_quantity,
				count(distinct
			...
			from "airup_eu_dwh"."dbt_feldm"."dim_shopify_product_cohort_spine" spine
				left join first_order_per_product_and_subsequent_orders a
					on spine.date_spine = a.first_created_at::date
...
price_tier as 
(select distinct subcategory_3_clean, max(pod_mother_price_group) over (partition by subcategory_3_clean) as price_tier
...
from
	final
left join price_tier pt
on final.product = pt.subcategory_3_clean
where
